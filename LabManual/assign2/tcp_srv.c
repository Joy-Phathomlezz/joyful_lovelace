...
void add_customer(struct sockaddr_in *cliaddr) {
    char ip[32]; strcpy(ip, inet_ntoa(cliaddr->sin_addr));
    int port = ntohs(cliaddr->sin_port);
    for(int i=0;i<unique_customers;i++)
        if(strcmp(customers[i].ip,ip)==0 && customers[i].port==port) return;
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        int connfd = accept(listenfd, (struct sockaddr *)&cliaddr, &clen);
        if (connfd < 0) { perror("accept"); continue; }
        add_customer(&cliaddr);

        char buf[BUF_SIZE], reply[BUF_SIZE], fruit[32];
        int qty;
        ...
        // Compose customer list and count
        char custlist[BUF_SIZE] = "Transacted customer(s): ";
        for (int i = 0; i < unique_customers; ++i) {
            char temp[64];
        ...
        printf("To %s:%d:\n%s", inet_ntoa(cliaddr.sin_addr), ntohs(cliaddr.sin_port), buf);
        close(connfd);
    }
    close(listenfd);
    return 0;
